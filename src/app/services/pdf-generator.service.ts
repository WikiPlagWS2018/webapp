...
import { WikiExcerpt } from '../models/wiki-excerpt';
...
export class PdfGeneratorService {
  ...
  printLinksOfPlagiarism(doc: any) {
    this.test = this.plagResponse.plags[this.printedPlagiarismCounter - 1].wiki_excerpts;

    for (let j = 0; j < this.test.length; j++) {
      const link = '--> https://de.wikipedia.org/?curid=' + this.test[j].id;
